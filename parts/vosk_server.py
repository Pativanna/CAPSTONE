...
from parts.voice_commands import STRICT_COMMAND_PHRASES, match_strict_command
...
for phrase, command in STRICT_COMMAND_PHRASES:
    COMANDOS_ENTRENAMIENTO.setdefault(command, []).append(phrase)
...
def _comando_desde_texto(texto: str) -> str:
    cmd, _ = match_strict_command(texto, allow_partial=True)
    return cmd or ''
...
async def process_audio_stream(websocket, path):
    ...
    # RECOGNIZER COMANDOS (KWS/Gramática): estricto con "ingreso" para evitar falsos positivos
    ...
    COMMAND_GRAMMAR = json.dumps([phrase for phrase, _ in STRICT_COMMAND_PHRASES])
    try:
    ...
    last_cmd_sent_at = {cmd: 0 for cmd in COMANDOS_ENTRENAMIENTO.keys()}
    ...
    def normalize_command(text: str):
        cmd, _ = match_strict_command(text, allow_partial=True)
        return cmd
    ...
    async def maybe_send_command(text: str):
        ...
        if cmd in {'finalizar_proceso', 'cancelar_proceso'}:
            ...
            if candidate:
                import re
                patterns = [
                    r"iniciar(\s+el)?\s+ingreso",
                    r"inicio\s+ingreso",
                    r"detener(\s+el)?\s+ingreso",
                    r"finalizar(\s+el)?\s+ingreso",
                    r"cancelar(\s+el)?\s+ingreso",
                    r"repetir(\s+el)?\s+ingreso",
                    r"confirmar(\s+el)?\s+ingreso",
                    r"ingreso"
                ]
                for pat in patterns:
...
async def handle_train_muestra(request):
    ...
    Acepta multipart/form-data o JSON con:
      - usuario_id (int)
      - comando (clave interna: iniciar_proceso | finalizar_proceso | cancelar_proceso | confirmar_datos | repetir_proceso)
