(function(){
  ...
  function logBluetooth(evento, datos={}){
    if (!IS_AUTHENTICATED || bluetoothLogSilenced) return;
    try {
      const payload = {
        evento,
        datos,
      ...
      fetch(LOG_ENDPOINT, {
        ...
        if (resp.status === 403){
          bluetoothLogSilenced = true;
          console.warn('Bluetooth log bloqueado por el servidor (403). Se desactivan futuros envíos.');
        }
      }).catch(err => {
        console.warn('BT log fetch falló:', err && err.message);
      });
    } catch(_){ }
  }
  ...
  this._cola = this._cargarCola();
    ...
    async _restaurarConexionPrevia(){
      try {
        if (!this._shouldUseBluetooth()) {
          this._clearPersistedConnection();
          logBluetooth('restore_skipped_inactive_method');
          return;
        }
        ...
        if (!wasConnected || !info.name) {
          return;
        }
        if (typeof navigator === 'undefined' || !navigator.bluetooth) {
          logBluetooth('restore_skipped_no_bluetooth_api');
          return;
        }
        ...
        const devices = await bluetoothApi.getDevices();
        const target = devices.find(d => d.id === info.id || d.name === info.name);
        if (!target) {
          logBluetooth('restore_device_not_found', { info });
          return;
        }
    ...
    async _conectarDispositivo(device, { silencioso }){
      ...
      this._onGattDisc = () => {
        this._handleGattDisconnect({
          reason: 'gatt_disconnected',
          meta: { deviceName: device && device.name }
        });
      };
  ...
  logBluetooth('characteristic_ok');
    ...
    async disconnect(){
      const meta = this.getDeviceName ? { deviceName: this.getDeviceName() } : undefined;
      try {
        if (this._cliente && this._cliente.device) {
          try {
            this._cliente.device.removeEventListener('gattserverdisconnected', this._onGattDisc);
          } catch(_){ }
          if (this._cliente.device.gatt && this._cliente.device.gatt.connected) {
            this._cliente.device.gatt.disconnect();
          }
        }
      } finally {
        this._handleGattDisconnect({ reason: 'manual_disconnect', meta });
      }
    }
    ...
    _clearPersistedConnection(){
      try { localStorage.setItem('bluetoothPrinterConnected', 'false'); } catch(_){ }
      if (!this._shouldUseBluetooth()) {
        try { localStorage.removeItem('bluetoothPrinterDevice'); } catch(_){ }
      }
    }

    _shouldUseBluetooth(){
      try {
        return (localStorage.getItem('printerMethod') || 'usb') === 'bluetooth';
      } catch(_){
        return false;
      }
    }

    _handleGattDisconnect({ reason = 'gatt_disconnected', meta } = {}){
      if (this._disconnectHandled) return;
      this._disconnectHandled = true;
      let lastDeviceName = null;
      if (this._cliente) {
        if (this._cliente.device) {
          lastDeviceName = this._cliente.device.name || null;
          try {
            if (this._onGattDisc) {
              this._cliente.device.removeEventListener('gattserverdisconnected', this._onGattDisc);
            }
        ...
        this._cliente.isConnected = false;
        this._cliente.device = null;
        this._cliente.server = null;
        this._cliente.characteristic = null;
      }
      this._dispositivo = null;
      this._clearPersistedConnection();
      this._emit({ type: 'disconnected', message: 'Desconectado' });
      const payload = meta ? { ...meta } : {};
      if (lastDeviceName && !payload.deviceName) {
        payload.deviceName = lastDeviceName;
      }
      logBluetooth(reason, payload);
    }
