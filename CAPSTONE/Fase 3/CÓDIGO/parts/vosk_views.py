...
- Los comandos "iniciar proceso", "detener", "finalizar", "guardar pieza", "confirmar" NO deben formar parte de los datos.
...
"""
        ...
        # OpenAI GPT-4 - ÚNICO BACKEND (Ollama eliminado)
        ...
        try:
            ...
            # Metadata tokens/costo si disponible
            try:
            ...
            extracted_json = json.dumps({"error": str(e)})
            ok = False
            codigo_http = 500
            registrar_llamada(
                tipo='chat', modelo='gpt-4', inicio_monotonic=inicio, ok=False, codigo_http=codigo_http,
            ...
            raise
        registrar_llamada(
            tipo='chat', modelo='gpt-4', inicio_monotonic=inicio, ok=ok, codigo_http=codigo_http,
            error_texto='' if ok else extracted_json, tokens_prompt=tokens_p, tokens_respuesta=tokens_c,
            prompt_texto_para_hash=transcript, origen='vosk_views.extract_from_transcript', request_id=request_id,
            meta={'modo': 'campos'}
        )
        ...
        # Si vienen textos con ruido, intentar extraer el primer bloque JSON
...
def log_transcription(request):
    ...
    Detecta comandos "iniciar proceso" y "finalizar proceso" para marcar is_capturing
    """
    try:
        ...
        # Detectar comandos de inicio/fin/confirmación de captura
        text_lower = text.lower()
        is_start_command = any(cmd in text_lower for cmd in ['iniciar proceso', 'iniciar el proceso', 'inicio proceso', 'inicia el proceso'])
        is_end_command = any(cmd in text_lower for cmd in ['detener proceso', 'finalizar proceso', 'terminar proceso', 'detener el proceso', 'finalizar el proceso', 'terminar el proceso'])
        is_confirm_command = any(cmd in text_lower for cmd in ['confirmar datos', 'confirmar los datos', 'confirmar', 'correcto', 'acepto', 'ok', 'sí', 'si'])
    ...
    try:
        ...
        try:
            ...
            if command == 'iniciar_proceso':
                ...
                session_obj.save(update_fields=['is_capturing','estado_grabacion'])
            elif command in ['repetir_proceso', 'repetir']:
                session_obj.is_capturing = True
                session_obj.estado_grabacion = VoiceSession.EstadoGrabacion.INICIADO
                session_obj.save(update_fields=['is_capturing','estado_grabacion'])
...
def transcribe_hybrid(request):
    ...
    1. Recibe audio completo capturado entre "iniciar proceso" y "finalizar proceso"
...
Responde SOLO con JSON válido, sin texto adicional:
{
  ...
  "observaciones": "string o null"
    ...
    try:
        # Usar OpenAI GPT-4 (ÚNICO BACKEND)
        client = OpenAI(api_key=settings.OPENAI_API_KEY)
